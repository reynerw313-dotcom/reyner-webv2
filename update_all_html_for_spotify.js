...
const css_link = '<link rel="stylesheet" href="assets/css/player.css">';

const new_player_html = `
...
files.forEach(filename => {
    ...
    if (!content.includes('assets/css/player.css')) {
    ...
    const pattern = /<!-- FLOATING SPOTIFY PLAYER -->[\s\S]*?<!-- MINIMIZED PLAYER TOGGLE -->\s*<button id="maximize-player"[\s\S]*?<\/button>/g;
    const pattern2 = /<div id="spotify-player-container"[\s\S]*?<button id="maximize-player"[\s\S]*?<\/button>/g;
    
    if (pattern.test(content)) {
        content = content.replace(pattern, new_player_html.trim());
    } else if (pattern2.test(content)) {
        content = content.replace(pattern2, new_player_html.trim());
    } else {
        if (content.includes('<!-- TOAST CONTAINER FOR ACHIEVEMENTS -->')) {
...
console.log('All HTML files updated successfully with new Spotify player layout and CSS link.');
