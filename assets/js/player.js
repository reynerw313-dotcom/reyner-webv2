...
window.addEventListener('pageChanged', () => {
    ...
    // Re-bind the navbar mute buttons after page swap
    MuteManager.bindButtons();
});

// ============================================================
// MUTE MANAGER — syncs navbar sound-toggle to music audio
// ============================================================
const MuteManager = {
    /**
     * Toggle mute for the music player audio element.
     * Does NOT pause, restart, or create a new Audio element.
     */
    toggleMute() {
        ...
        // Apply to the single audio instance
        if (player.audio) {
            player.audio.volume = player.isMuted ? 0 : (player.lastVolume / 100);
        }

        // Sync volume slider in the player UI
        const volumeSlider = document.getElementById('player-volume');
        if (volumeSlider) {
            volumeSlider.value = player.isMuted ? 0 : player.lastVolume;
            volumeSlider.style.setProperty('--fill-percent', `${player.isMuted ? 0 : player.lastVolume}%`);
        }

        // Update player volume button icon
        player.updateVolumeIcon();

        // Persist state
        player.saveState();

        // Update navbar sound icons
        this.updateIcons(player.isMuted);

        // Also mute UI click sounds
        if (window.SoundManager) {
            window.SoundManager.muted = player.isMuted;
            if (window.AppStorage) window.AppStorage.set('muted', player.isMuted);
            else localStorage.setItem('muted', player.isMuted);
        }
    },
    ...
    updateIcons(isMuted) {
        ...
        const mutedClass  = 'fa-solid fa-volume-xmark text-red-400';
        const activeClass = 'fa-solid fa-volume-high';

        [desktopIcon, mobileIcon].forEach(icon => {
            if (!icon) return;
            icon.className = isMuted ? mutedClass : activeClass;
        });
    },

    bindButtons() {
        ['sound-toggle-desktop', 'sound-toggle-mobile'].forEach(id => {
            const btn = document.getElementById(id);
            if (btn && !btn._muteBound) {
                btn.addEventListener('click', () => this.toggleMute());
                btn._muteBound = true;
            }
        });

        // Sync icon state with whatever the player's current mute state is
        const player = window.GlobalPlayer;
...
document.addEventListener('DOMContentLoaded', () => {
    // Wait a tick so PlayerManager finishes init first
    setTimeout(() => MuteManager.init(), 100);
});
