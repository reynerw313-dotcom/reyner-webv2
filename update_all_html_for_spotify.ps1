$files = @("index.html", "profil.html", "informatika.html", "bahasa.html", "games.html", "playlist.html", "stats.html")
...
foreach ($file in $files) {
    if (Test-Path $file) {
        ...
        # 1. CSS Link Injection
        if ($content -notlike "*assets/css/player.css*") {
            if ($content -like "*<link rel=`"stylesheet`" href=`"style.css`">*") {
                $content = $content -replace '<link rel="stylesheet" href="style.css">', "<link rel=`"stylesheet`" href=`"style.css`">`r`n  $css_link"
            } else {
                $content = $content -replace '</head>', "  $css_link`r`n</head>"
            }
        }

        # 2. Player HTML Replacement
        $pattern = '(?s)<!-- FLOATING SPOTIFY PLAYER -->.*?<!-- MINIMIZED PLAYER TOGGLE -->\s*<button id="maximize-player".*?</button>'
        $pattern2 = '(?s)<div id="spotify-player-container".*?<button id="maximize-player".*?</button>'
        
        if ($content -match $pattern) {
            $content = [regex]::Replace($content, $pattern, $new_player_html)
        } elseif ($content -match $pattern2) {
            $content = [regex]::Replace($content, $pattern2, $new_player_html)
        } else {
            if ($content -like "*<!-- TOAST CONTAINER FOR ACHIEVEMENTS -->*") {
                $content = $content -replace '<!-- TOAST CONTAINER FOR ACHIEVEMENTS -->', "$new_player_html`r`n            <!-- TOAST CONTAINER FOR ACHIEVEMENTS -->"
            }
        }
