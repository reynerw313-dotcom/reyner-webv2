...
theme_sound_toggles_desktop = """
          <!-- Theme & Sound Toggles -->
          <div class="flex items-center gap-2 border-r border-slate-800 pr-3 mr-1">
...
theme_sound_toggles_mobile = """
        <!-- Theme & Sound Toggles Mobile -->
...
for filename in files:
    ...
    # 1. Replace the native music player block
    old_player_regex = r'<!-- NATIVE FLOATING MUSIC PLAYER -->\s*<div id="music-player-container"[\s\S]*?</div>\s*</div>'
    if re.search(old_player_regex, content):
        content = re.sub(old_player_regex, spotify_player_html.strip(), content)
    elif '<!-- FLOATING SPOTIFY PLAYER -->' not in content:
        if '<!-- TOAST CONTAINER FOR ACHIEVEMENTS -->' in content:
            content = content.replace('<!-- TOAST CONTAINER FOR ACHIEVEMENTS -->', spotify_player_html + '\n            <!-- TOAST CONTAINER FOR ACHIEVEMENTS -->')
            
    # 2. Add Theme and Sound Toggles to Desktop Nav
    if 'id="theme-toggle-desktop"' not in content:
        desktop_target = r'(<!-- Social Media Shortcuts \(Header Right\) -->\s*<div class="hidden md:flex items-center gap-3">)'
        content = re.sub(desktop_target, theme_sound_toggles_desktop + r'\n        \1', content)

    # 3. Add Theme and Sound Toggles to Mobile Nav
    if 'id="theme-toggle-mobile"' not in content:
        mobile_target = r'(<!-- Social Media Icons in Mobile Menu -->)'
        content = re.sub(mobile_target, theme_sound_toggles_mobile + r'\n        \1', content)
        
    with open(filename, 'w', encoding='utf-8') as f:
        f.write(content)

print("All HTML files updated with Spotify Player and Toggles.")
